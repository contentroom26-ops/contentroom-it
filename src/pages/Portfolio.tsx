...
const PortfolioSectionVideoBackground = () => (
  <div className="absolute inset-0 overflow-hidden pointer-events-none">
    <video autoPlay loop muted playsInline className="absolute inset-0 w-full h-full object-cover" src="/global-bg.mp4" />
    <div className="absolute inset-0" style={{ background: "hsl(43 100% 98% / 0.55)" }} />
    <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse at center, transparent 30%, hsl(43 100% 98% / 0.7) 100%)" }} />
  </div>
);
...
export const cases: CaseItem[] = [
  ...
  { slug: "shade", img: portfolioShade, client: "Shade", service: "Content & Production", category: "Video", result: "Lancio singolo \"Toxic\"" },
  { slug: "jason-derulo", img: portfolioJasonDerulo, client: "Jason Derulo", service: "Photo & Video", category: "Video", result: "Copertura esclusiva live" },
];
          ...
          <div className="max-w-6xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}>
              <div className="flex items-center gap-4 mb-6">
                ...
                <p className="font-body text-xs font-bold tracking-[0.4em] uppercase text-brand-orange">Portfolio</p>
              </div>
              <h1 className="font-display font-black tracking-tight leading-[1.05] mb-6" style={{ fontSize: "clamp(2.5rem, 6vw, 5rem)" }}>
                Lavori,<br /><span className="text-primary">non promesse.</span>
              </h1>
            ...
            <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
              <AnimatePresence mode="popLayout">
                {visible.map((c, i) => (
                  ...
                  >
                    <Link to={`/portfolio/${c.slug}`} className="group relative block rounded-2xl overflow-hidden cursor-pointer aspect-[4/3]">
                      <img src={c.img} alt={c.client} loading="lazy" className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                      ...
                      <div className="absolute inset-0 bg-black/75 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center p-6">
                        <div className="text-center">
                          <p className="font-body text-[10px] tracking-[0.4em] uppercase mb-3" style={{ color: CELESTE }}>{c.service}</p>
                          <h3 className="font-display font-bold text-2xl text-white mb-2">{c.client}</h3>
                          <p className="font-body text-xs text-white/65 tracking-wider uppercase">Scopri il case study →</p>
                        </div>
                      </div>
                      <div className="absolute bottom-0 left-0 p-6 group-hover:opacity-0 transition-opacity duration-300">
                        <p className="font-display font-bold text-sm mb-1" style={{ color: CELESTE }}>{c.result}</p>
...
export default Portfolio;import { useState, useMemo } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { Link } from "react-router-dom";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
